...
#SBATCH --job-name=fothi_skel_ai
#SBATCH --output=slurm_out/%j_ai.out
#SBATCH --error=slurm_out/%j_ai.err
#SBATCH --partition=ai
#SBATCH --nodes=1
#SBATCH --gres=gpu:8
...
singularity exec --nv \
  ...
  -B /project/c_mvcondif/venvs/:/venvs/ \
    ...
    -B /project/c_mvcondif/outputs/:/outputs/ \
      ...
      python train.py --base configs/zero123plus-finetune-sk.yaml --gpus 0,1,2,3,4,5,6,7 --num_nodes 1 --name "fothi_skel_ai_$(date +%m%d_%H%M)" --seed 42