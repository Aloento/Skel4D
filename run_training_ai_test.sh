...
#SBATCH --output=slurm_out/%j_sk_ai.out
#SBATCH --error=slurm_out/%j_sk_ai.err
#SBATCH --partition=ai
#SBATCH --nodes=1
#SBATCH --gres=gpu:2
...
singularity exec --nv \
  ...
  -B /project/c_mvcondif/venvs/:/venvs/ \
    ...
    -B /project/c_mvcondif/outputs/:/outputs/ \
      ...
      python train.py --base configs/zero123plus-finetune-sk.yaml --gpus 0,1 --num_nodes 1 --name "fothi_sk_ai_$(date +%m%d_%H%M)" --seed 42